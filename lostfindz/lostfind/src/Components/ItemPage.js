...
import toast, { useToaster } from "react-hot-toast"; // Add this import
...
function ItemPage(props) {
  ...
  const submitActivate = () => {
    Axios({
      method: "POST",
      url: `http://localhost:5000/activateItem/${itemid}`,
    })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    //const item_id = ""; // Add the necessary value for item_id
    const currentURL = window.location.href;
    console.log("Current URL is:", currentURL);
          
          const urlParams = new URLSearchParams(currentURL);
          const match = /[\?&]cid=([^&]+)/.exec(currentURL);
      const item_id = match && decodeURIComponent(match[1].replace(/\+/g, ' '));
      console.log("Item ID is:", item_id);
          const itemType = urlParams.get("type");
          const parts = currentURL.split('/');
      const lastPart = parts[parts.length - 1];

      // Set the current_user based on the last part of the URL
      const current_user = lastPart === "true" ? "true" : "false";
  ...
  const delete_item = () => {
    ...
    Axios({
      ...
      method: "POST",
      data: { item_id: itemid }, // Add the necessary key-value pair
    })
      .then((response) => {
  ...
  const handleEdit = () => {
    ...
    if (newitemimage.length > 0) {
      ...
      itemimage.map((image) => {
        console.log(image);
        info.append("itemPictures", image.img); // Append old images to itemPictures
      });
    }
